...
export async function POST(request) {
  ...
  if (!body?.email || !body?.status) {
    return NextResponse.json({ code: 'OTP_REQUEST_VALIDATION' }, { status: 400 })
  }
  try {
    const response = await fetch(`${API_BASE_URL}${apiPath('WATHIQ_OTP_REQUEST_PATH', '/api/v1/auth/otp/request')}`, { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'application/json' }, body: JSON.stringify({ email: body.email,status: body.status, }), cache: 'no-store' })
